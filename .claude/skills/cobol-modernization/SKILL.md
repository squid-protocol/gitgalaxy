...
- **Run both audits locally, not just ruff.** `ruff_audit.py --ci` and `mypy_audit.py --ci`, plus the dead-key audit when you add dict-keyed reads. #3640 reached CI with 5 mypy findings; the usual cause is an Optional that is never narrowed. Or run everything: `python tests/tools/preflight.py`.
