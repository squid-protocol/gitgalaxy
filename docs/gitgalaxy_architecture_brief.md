...
| **Timestamp** | `2026-07-31T03:55:18.346839+00:00` |
| **Scan Duration** | `7.97s` |
| **Git Branch** | `main` |
| **Git Commit** | `a4bdc9560bc48dc369d811b735707ed9ee558731` |
...
| % Scanned of codebase = | 26.8% |
| Dominant Lang | PYTHON |

## 3.5 MACRO-NETWORK TOPOLOGY (Resilience & Coupling)
| Metric | Value | Interpretation |
|---|---|---|
| Modularity | 0.6408 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | -0.1993 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.0% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 2.8952 | Hops between files. Lower = Tighter coupling. |
| Articulation Pts | 36 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| PYTHON | 139 | 50878 | 79.9% |
| MARKDOWN | 24 | 0 | 13.8% |
| YAML | 8 | 481 | 4.6% |
| PLAINTEXT | 3 | 0 | 1.7% |
...
> **Architectural Drift Z-Score:** `5.118`
...
|---|---|---|
| file_cluster_8 | 103 | 59.2% |
| file_cluster_13 | 29 | 16.7% |
| file_cluster_0 | 8 | 4.6% |
...
| Category | Count | Repo % |
|---|---|---|
| Static: Literature & Documentation | 27 | 15.5% |
...
| Error & Exception Exposure | 0.0 | 76.6 | 6.4 | 0.0 | 0.0 |
...
| Volatility Exposure | 0.0 | 94.6 | 9.8 | 0.0 | 0.0 |
...
1. **language_standards.py** (`gitgalaxy/standards/language_standards.py`) — 24 inbound connections
...
4. **_extraction_harness.py** (`tests/extraction/_extraction_harness.py`) — 7 inbound connections
...
- **Unknown Dependencies:** `923` packages imported that bypass the Zero-Trust whitelist.
...
### 2. `gitgalaxy/metrics/statistical_auditor.py` (PYTHON) -> Cumulative Risk: **685.54**
...
### 3. `gitgalaxy/galaxyscope.py` (PYTHON) -> Cumulative Risk: **674.15**
- **Archetype:** `file_cluster_8` (Distance: 12.542 IQR)
- **Magnitude:** 3862.46 | **LOC:** 3044 | **CtrlFlow:** 71.1% | **Authorship Centralization:** 63.6%
- **Primary Risk Drivers:** Spec Match (100.0%), Algorithmic Dos (100.0%), Logic Bomb (100.0%), Churn (94.64%)
...
### 5. `gitgalaxy/cobol_refractor_controller.py` (PYTHON) -> Cumulative Risk: **663.1**
...
### 6. `gitgalaxy/tools/cobol_to_cobol/cobol_compiler_forge.py` (PYTHON) -> Cumulative Risk: **662.16**
...
### 7. `gitgalaxy/standards/config_resolver.py` (PYTHON) -> Cumulative Risk: **654.54**
- **Archetype:** `Unknown Archetype` (Distance: N/A IQR)
...
### 8. `gitgalaxy/metrics/chronometer.py` (PYTHON) -> Cumulative Risk: **649.34**
...
### 10. `gitgalaxy/recorders/sarif_recorder.py` (PYTHON) -> Cumulative Risk: **637.26**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 0):` pytest, ccall, qualified, Reactive.Banana, network, re, kotlin.collections.List, data.csv...
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 7.789
  * `Choke Point (Betweenness):` 0.00289 | `Ripple Effect (Closeness):` 0.017341
  * `Imports (Out-Degree: 30):` concurrent.futures, importlib.util, argparse, gitgalaxy.core.spatial_correlation, re, gitgalaxy.state_rehydrator, collections, gitgalaxy.recorders.sarif_recorder...
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 9.949
  * `Choke Point (Betweenness):` 6.2e-05 | `Ripple Effect (Closeness):` 0.028324
  * `Imports (Out-Degree: 1):` logging, networkx.algorithms, networkx, gitgalaxy.standards.analysis_lens, warnings, typing, pathlib, math...
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 9.96
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.033631
  * `Imports (Out-Degree: 0):` logging, statistics, typing, math, re, gitgalaxy.standards, os
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 7.456
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.018064
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` logging, time, typing, pathlib, math, re, gitgalaxy.standards.language_standards, gitgalaxy.standards.gitgalaxy_config
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `tests/extraction/test_class_extraction_strict.py` (PYTHON | Tier 1.5 | AI Safe: 0.0%)
- **Global Archetype:** `Unknown Archetype` (Drift: N/A IQR)
- **Magnitude:** 953.51 | **LOC:** 237 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* None
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 5.905
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.023121
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 5.628
  * `Choke Point (Betweenness):` 0.000146 | `Ripple Effect (Closeness):` 0.023121
  * `Imports (Out-Degree: 3):` logging, gitgalaxy.standards.analysis_lens, bisect, gitgalaxy.core.spatial_correlation, time, typing, math, re...
  * `Imported By (In-Degree: 3):` (Excluded from Brief to save tokens)

### `tests/extraction/test_dependency_extraction_strict.py` (PYTHON | Tier 1.5 | AI Safe: 0.0%)
- **Global Archetype:** `Unknown Archetype` (Drift: N/A IQR)
- **Magnitude:** 822.56 | **LOC:** 345 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* None
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` cats.effect.IO, pytest, Control.Monad, UIKit, static, qualified, machine, std.core...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 7.456
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.018064
  * `Imports (Out-Degree: 0):` logging, heapq, sqlite3, statistics, typing, pathlib, gitgalaxy.standards, collections...
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` concurrent.futures, or, re, os, logging, gitgalaxy.standards.analysis_lens, gitgalaxy.galaxyscope, failure...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 7.456
  * `Choke Point (Betweenness):` 3.4e-05 | `Ripple Effect (Closeness):` 0.018064
  * `Imports (Out-Degree: 1):` logging, time, typing, pathlib, gitgalaxy.standards, shutil, subprocess, os...
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` logging, gitgalaxy.core.detector, pytest, gitgalaxy.core.prism, math, re, unittest.mock, gitgalaxy.standards.language_standards...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` logging, xgboost, networkx, gitgalaxy.standards.analysis_lens, typing, pathlib, collections, numpy...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` com.example.MyClass, statement, pytest, static, typing, pathlib, gitgalaxy.standards.language_standards, _extraction_harness...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 7.235
  * `Choke Point (Betweenness):` 0.000302 | `Ripple Effect (Closeness):` 0.00578
  * `Imports (Out-Degree: 9):` datetime, gitgalaxy.tools.cobol_to_cobol.cobol_dag_architect, gitgalaxy.tools.cobol_to_cobol.cobol_jcl_auditor, argparse, sqlite3, gitgalaxy.tools.cobol_to_cobol.cobol_lexical_patcher, gitgalaxy.licensing, typing...
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` argparse, typing, pathlib, sys, ast, json
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` dataclasses, logging, __future__, copy, typing, gitgalaxy.standards, yaml
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `gitgalaxy/standards/language_standards.py` (PYTHON | Tier 1.5 | AI Safe: 0.0%)
- **Global Archetype:** `Unknown Archetype` (Drift: N/A IQR)
- **Magnitude:** 352.72 | **LOC:** 12242 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* None
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` keyword., type, inside, path, typing, re, java.util.
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` pytest, identity, tempfile, which, gitgalaxy.recorders.sarif_recorder, os, gitgalaxy.metrics.signal_processor, json
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` gitgalaxy.tools.supply_chain_security.supply_chain_firewall, pytest, policy, pathlib, unittest.mock, was, bypassed, multiprocessing...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 3.911
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 6):` gitgalaxy.tools.cobol_to_java.cobol_to_java_agent_forge, org.slf4j.Logger, gitgalaxy.tools.cobol_to_java.cobol_to_java_api_contract_forge, org.slf4j.LoggerFactory, argparse, gitgalaxy.licensing, gitgalaxy.tools.cobol_to_java.cobol_to_java_decoder_forge, gitgalaxy.tools.cobol_to_java.cobol_to_java_build_forge...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 9.118
  * `Choke Point (Betweenness):` 0.000269 | `Ripple Effect (Closeness):` 0.023121
  * `Imports (Out-Degree: 4):` logging, datetime, uuid, gitgalaxy.standards.language_lens, gitgalaxy.standards.gitgalaxy_config, gitgalaxy.security.manifest_parser, typing, pathlib...
  * `Imported By (In-Degree: 3):` (Excluded from Brief to save tokens)
...
- `gitgalaxy/recorders/llm_recorder.py` -> Churn: **71.54%** | Cog Load: 62.8023% | Debt: 0.0%
...
- `gitgalaxy/galaxyscope.py` -> **Severity: 0.224** (Bridge: 0.0029 * Flux: 77.6198%)
...
- `gitgalaxy/standards/config_resolver.py` -> **Severity: 4.955** (Embedded: 0.0619 * Error Risk: 80.0%)
- `gitgalaxy/core/network_risk_sensor.py` -> **Severity: 2.062** (Embedded: 0.0283 * Error Risk: 72.8082%)
- `gitgalaxy/recorders/sarif_recorder.py` -> **Severity: 1.46** (Embedded: 0.0231 * Error Risk: 63.1492%)
- `gitgalaxy/tools/ai_guardrails/ai_appsec_sensor.py` -> **Severity: 1.445** (Embedded: 0.0181 * Error Risk: 80.0%)
- `gitgalaxy/recorders/llm_recorder.py` -> **Severity: 1.384** (Embedded: 0.0181 * Error Risk: 76.6083%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `gitgalaxy/standards/config_resolver.py` -> **Severity: 3066.6** (Blast Radius: 30.666 * Doc Risk: 100.0%)
- `gitgalaxy/standards/language_standards.py` -> **Severity: 1014.768** (Blast Radius: 59.845 * Doc Risk: 16.9566%)
- `gitgalaxy/metrics/tensor_scanner.py` -> **Severity: 745.6** (Blast Radius: 7.456 * Doc Risk: 100.0%)
- `gitgalaxy/tools/cobol_to_cobol/cobol_compiler_forge.py` -> **Severity: 723.5** (Blast Radius: 7.235 * Doc Risk: 100.0%)
- `gitgalaxy/tools/network_auditing/full_api_network_map.py` -> **Severity: 688.577** (Blast Radius: 7.456 * Doc Risk: 92.352%)
