...
| **Timestamp** | `2026-08-13T19:18:09.064050+00:00` |
| **Scan Duration** | `8.23s` |
| **Git Branch** | `main` |
| **Git Commit** | `d4c4bfd83b54630c6f2ad75afc538d5536a6abfe` |
...
| % Scanned of codebase = | 33.9% |
| Dominant Lang | PYTHON |

## 3.5 MACRO-NETWORK TOPOLOGY (Resilience & Coupling)
| Metric | Value | Interpretation |
|---|---|---|
| Modularity | 0.7383 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | -0.3747 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.0% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 4.2942 | Hops between files. Lower = Tighter coupling. |
| Articulation Pts | 42 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| PYTHON | 232 | 55143 | 85.6% |
| MARKDOWN | 27 | 0 | 10.0% |
| YAML | 8 | 482 | 3.0% |
| PLAINTEXT | 3 | 0 | 1.1% |
| SHELL | 1 | 22 | 0.4% |
...
> **Architectural Drift Z-Score:** `4.985`
...
|---|---|---|
| file_cluster_8 | 150 | 55.4% |
| file_cluster_13 | 63 | 23.2% |
| file_cluster_0 | 19 | 7.0% |
...
- `.md`: 352x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 1x Excluded (Machine-Generated Source Code Signature: 58 LOC), 1x Excluded (Machine-Generated Source Code Signature: 41 LOC)
...
| Instability Exposure | 0.0 | 10.3 | 0.4 | 0.0 | 0.0 |
...
3. **detector.py** (`gitgalaxy/core/detector.py`) — 9 inbound connections
...
- `_get_node_name` (@ `tests/tools/tree_sitter_accuracy_audit.py`) -> Impact: **665.9** | LOC: 1197
  * *Intent:* # #1295: zig's error sets (`const Foo = error{...};`) get their own `ErrorSetDecl` node # class_start regex intentionally matches (its comment says so...
...
- `test_detector_exact_loc_mapping` (@ `tests/core_engine/test_detector.py`) -> Impact: **119.5** | LOC: 277
...
- `_gg_positions_in` (@ `tests/tools/class_start_diff.py`) -> Impact: **108.4** | LOC: 155

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `tests/extraction/languages` | 92 | 12862.9 | 3.34% | 0.0% |
| `tests/core_engine` | 19 | 2100.64 | 2.95% | 0.0% |
| `gitgalaxy` | 6 | 1738.18 | 8.61% | 0.0% |
| `gitgalaxy/core` | 9 | 1138.32 | 15.2% | 7.19% |
...
- `gitgalaxy/core/prism.py` -> **39.5138%** Exposure
...
- `gitgalaxy/core/prism.py` -> **99.9643%** Exposure
...
- **Unknown Dependencies:** `1560` packages imported that bypass the Zero-Trust whitelist.

## 11. CUMULATIVE RISK HITLIST (Top 10 Highest Risk Files)
> Cumulative Risk is the sum of all individual risk exposures. These files represent the highest multi-dimensional technical debt and architectural fragility.

### 1. `gitgalaxy/core/prism.py` (PYTHON) -> Cumulative Risk: **528.17**
- **Archetype:** `file_cluster_16` (Distance: 11.953 IQR)
- **Magnitude:** 360.36 | **LOC:** 977 | **CtrlFlow:** 63.2% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9643%), Safety Score (83.3336%), Verification (80.0%)
- **Heaviest Functions:** `_find_balanced_end` (Impact: 100.4), `_partition_embedded_languages` (Impact: 39.6), `_strip_single_line_comments` (Impact: 28.4)

### 2. `gitgalaxy/recorders/llm_recorder.py` (PYTHON) -> Cumulative Risk: **464.65**
...
### 3. `gitgalaxy/core/detector.py` (PYTHON) -> Cumulative Risk: **462.13**
- **Archetype:** `file_cluster_8` (Distance: 11.526 IQR)
- **Magnitude:** 243.26 | **LOC:** 3601 | **CtrlFlow:** 74.8% | **Authorship Centralization:** 97.9%
- **Primary Risk Drivers:** Spec Match (100.0%), Churn (100.0%), Verification (80.0%), Safety Score (59.869%)
- **Heaviest Functions:** `_resolve_class_start_match` (Impact: 31.8), `_count_haskell_type_arrows` (Impact: 20.1), `get_token_mass` (Impact: 6.4)
...
### 5. `gitgalaxy/metrics/statistical_auditor.py` (PYTHON) -> Cumulative Risk: **438.83**
...
### 6. `gitgalaxy/galaxyscope.py` (PYTHON) -> Cumulative Risk: **419.98**
...
### 9. `gitgalaxy/core/network_risk_sensor.py` (PYTHON) -> Cumulative Risk: **395.47**
...
### 10. `gitgalaxy/recorders/gpu_recorder.py` (PYTHON) -> Cumulative Risk: **394.55**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 4.695
  * `Choke Point (Betweenness):` 0.001143 | `Ripple Effect (Closeness):` 0.011111
  * `Imports (Out-Degree: 29):` gitgalaxy.core.prism, gitgalaxy.recorders.sarif_recorder, gitgalaxy.metrics.statistical_auditor, gitgalaxy.security.manifest_parser, importlib.util, is, gitgalaxy.recorders.audit_recorder, gitgalaxy.recorders.llm_recorder...
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 0):` _strict_harness, pathlib, sys, pytest, gitgalaxy.standards.language_standards
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` _strict_harness, pathlib, sys, pytest, gitgalaxy.standards.language_standards, re
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` _strict_harness, pathlib, sys, legacy, pytest, gitgalaxy.standards.language_standards
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 4.499
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.011574
  * `Imports (Out-Degree: 0):` gitgalaxy.standards, statistics, sqlite3, pathlib, json, typing, collections, logging...
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` extraction._extraction_harness, pathlib, sys, pytest, gitgalaxy.standards.language_standards
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` only, pathlib, typing, sys, _extraction_harness, pytest, gitgalaxy.standards.language_standards
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` _strict_harness, pathlib, sys, and, pytest, gitgalaxy.standards.language_standards, this
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Global Archetype:** `Unknown Archetype` (Drift: N/A IQR)
- **Magnitude:** 489.32 | **LOC:** 2627 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_detector_exact_loc_mapping` (Impact: 119.5)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` gitgalaxy.core.prism, unittest.mock, gitgalaxy.standards.gitgalaxy_config, math, pytest, gitgalaxy.standards.language_standards, gitgalaxy.core.spatial_mapper, re...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` gitgalaxy.standards, passes, re, gitgalaxy.standards.analysis_lens, yaml, subprocess, tempfile, or...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` only, pathlib, typing, sys, java.util., _extraction_harness, pytest, gitgalaxy.standards.language_standards...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` pathlib, sys, _extraction_harness, pytest, gitgalaxy.standards.language_standards
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 8.014
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.025
  * `Imports (Out-Degree: 0):` logging, gitgalaxy.standards, statistics, typing, re, os, math
  * `Imported By (In-Degree: 6):` (Excluded from Brief to save tokens)

### `gitgalaxy/core/prism.py` (PYTHON | Tier 1.5 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_16` (Drift: 11.953 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 5.368 IQR)
- **Top Global Matches:** file_cluster_16: 11.953, file_cluster_8: 12.036, file_cluster_13: 12.261
- **Magnitude:** 360.36 | **LOC:** 977 | **CtrlFlow:** 63.2% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (40.852%), Tech Debt (39.5138%)
**Top Internal Functions/Classes:**
  * `_find_balanced_end` (Impact: 100.4)
  ...
  * `_strip_single_line_comments` (Impact: 28.4)
  ...
  * `callback` (Impact: 8.9)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 141`, `structural_boundaries: 82`, `args: 25`, `func_start: 22`, `class_start: 3`
* *Risk/State:* `safety_bypasses: 23`, `state_mutation: 117`, `fragile_debt: 6`
* *Architecture:* `api: 10`, `import: 4`
* *Defense:* `safety: 4`, `doc: 44`
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 8.673
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.025
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` logging, contextlib, pathlib, gitgalaxy.standards.gitgalaxy_config, typing, time, gitgalaxy.standards.language_standards, re...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 5.0
  * `Choke Point (Betweenness):` 1.8e-05 | `Ripple Effect (Closeness):` 0.014815
  * `Imports (Out-Degree: 1):` logging, gitgalaxy.standards.analysis_lens, token, pathlib, typing, networkx.algorithms, warnings, collections...
  * `Imported By (In-Degree: 3):` (Excluded from Brief to save tokens)
...
- **Top Global Matches:** file_cluster_8: 11.526, file_cluster_16: 11.805, file_cluster_7: 11.897
- **Magnitude:** 243.26 | **LOC:** 3601 | **CtrlFlow:** 74.8% | **Authorship Centralization:** 97.9%
- **Risk Profile:** Cognitive Load (14.9077%), Tech Debt (14.6155%)
**Top Internal Functions/Classes:**
  * `_resolve_class_start_match` (Impact: 31.8)
  * `_count_haskell_type_arrows` (Impact: 20.1)
    * *Intent:* # ALL of them are trailing modifiers (e.g., `return true if x unless y`)
...
* *Sec High Risk Execution:* 1 instances
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 654`, `structural_boundaries: 220`, `args: 30`, `func_start: 29`, `class_start: 6`
* *Risk/State:* `safety_bypasses: 56`, `state_mutation: 116`, `dead_code: 10`, `planned_debt: 1`, `fragile_debt: 9`
* *Architecture:* `api: 15`, `concurrency: 1`, `import: 13`
* *Defense:* `safety: 39`, `doc: 89`, `test: 2`, `immutability_locks: 2`
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 9.508
  * `Choke Point (Betweenness):` 0.000211 | `Ripple Effect (Closeness):` 0.035556
  * `Imports (Out-Degree: 2):` logging, gitgalaxy.standards.analysis_lens, typing, gitgalaxy.core.spatial_correlation, bisect, time, gitgalaxy.standards.language_standards, collections...
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` tempfile, gitgalaxy.recorders.sarif_recorder, which, json, pytest, os, gitgalaxy.metrics.signal_processor, identity...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` gitgalaxy.standards.analysis_lens, pathlib, numpy, typing, collections, pandas, xgboost, logging...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` _strict_harness, pathlib, sys, pytest, gitgalaxy.standards.language_standards, multiprocessing, re, shapes
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` subprocess, ast, sqlite3, tempfile, json, pathlib, typing, argparse...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- `gitgalaxy/core/prism.py` (PYTHON) | Magnitude: 360.36 | Delta: **0.083 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 448, branch: 141, state_mutation: 117, structural_boundaries: 82
...
- `gitgalaxy/core/state_rehydrator.py` (PYTHON) | Magnitude: 13.9 | Delta: **0.095 IQR** | Secondary Pull: `file_cluster_13`
...
- `gitgalaxy/recorders/llm_recorder.py` -> Churn: **56.76%** | Cog Load: 61.3945% | Debt: 0.0%
...
- `gitgalaxy/core/prism.py` -> **Joe Esquibel** (100.0% isolated ownership) | Magnitude: 360.36
...
- `gitgalaxy/core/detector.py` -> **Severity: 0.011** (Bridge: 0.0002 * Flux: 51.2866%)
...
- `gitgalaxy/core/detector.py` -> **Severity: 2.129** (Embedded: 0.0356 * Error Risk: 59.869%)
- `gitgalaxy/core/prism.py` -> **Severity: 2.083** (Embedded: 0.025 * Error Risk: 83.3336%)
- `gitgalaxy/core/spatial_correlation.py` -> **Severity: 1.985** (Embedded: 0.0339 * Error Risk: 58.6062%)
- `gitgalaxy/standards/gitgalaxy_config.py` -> **Severity: 1.934** (Embedded: 0.0346 * Error Risk: 55.9468%)
...
- `gitgalaxy/standards/analysis_lens.py` -> **Severity: 392.503** (Blast Radius: 22.906 * Doc Risk: 17.1354%)
- `gitgalaxy/standards/config_resolver.py` -> **Severity: 390.49** (Blast Radius: 21.839 * Doc Risk: 17.8804%)
- `gitgalaxy/standards/gitgalaxy_config.py` -> **Severity: 262.323** (Blast Radius: 14.671 * Doc Risk: 17.8804%)
- `scripts/update_golden_masters.sh` -> **Severity: 210.045** (Blast Radius: 2.357 * Doc Risk: 89.1154%)
- `gitgalaxy/core/detector.py` -> **Severity: 168.817** (Blast Radius: 9.508 * Doc Risk: 17.7553%)
