...
| **Timestamp** | `2026-07-28T20:28:22.865492+00:00` |
| **Scan Duration** | `7.05s` |
| **Git Branch** | `main` |
| **Git Commit** | `317dc1bb4dd19d510a590111680de36f3532f01f` |
...
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 471 |
| Total LOC | 46535 |
...
| Assortativity | -0.168 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.0% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 2.8435 | Hops between files. Lower = Tighter coupling. |
| Articulation Pts | 35 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| PYTHON | 129 | 46054 | 79.1% |
| MARKDOWN | 23 | 0 | 14.1% |
| YAML | 8 | 481 | 4.9% |
| PLAINTEXT | 3 | 0 | 1.8% |
...
> **Architectural Drift Z-Score:** `5.319`
...
|---|---|---|
| file_cluster_8 | 101 | 62.0% |
| file_cluster_13 | 24 | 14.7% |
| file_cluster_16 | 6 | 3.7% |
| file_cluster_0 | 5 | 3.1% |
| file_cluster_6 | 1 | 0.6% |

### Inert Structural Mass (Static Categories)
| Category | Count | Repo % |
|---|---|---|
| Static: Literature & Documentation | 26 | 16.0% |

## 5. EXCLUDED ARTIFACTS (Unparsable or Shielded Files)
*Total Excluded Artifacts: 471*

**Composition by Extension & Reason:**
- `.md`: 335x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 1x Excluded (Machine-Generated Source Code Signature: 63 LOC), 1x Excluded (Machine-Generated Source Code Signature: 34 LOC)
...
|---|---|---|---|---|---|
| Cognitive Load Exposure | 0.0 | 62.8 | 3.4 | 0.0 | 0.0 |
| Error & Exception Exposure | 0.0 | 76.6 | 6.9 | 0.0 | 0.0 |
| Tech Debt Exposure | 0.0 | 16.6 | 0.5 | 0.0 | 0.0 |
| Testing Exposure | 0.0 | 80.0 | 8.9 | 0.0 | 0.0 |
| API Exposure | 0.0 | 8.1 | 0.3 | 0.0 | 0.0 |
| Concurrency Exposure | 0.0 | 23.3 | 0.5 | 0.0 | 0.0 |
| State Flux Exposure | 0.0 | 100.0 | 9.6 | 0.0 | 0.0 |
| Commented Logic Exposure | 0.0 | 9.8 | 0.2 | 0.0 | 0.0 |
| Specification Exposure | 0.0 | 100.0 | 18.6 | 0.0 | 0.0 |
| Instability Exposure | 0.0 | 31.2 | 1.1 | 0.0 | 0.0 |
...
1. **language_standards.py** (`gitgalaxy/standards/language_standards.py`) — 16 inbound connections
...
4. **gitgalaxy_config.py** (`gitgalaxy/standards/gitgalaxy_config.py`) — 6 inbound connections
5. **signal_processor.py** (`gitgalaxy/metrics/signal_processor.py`) — 5 inbound connections

### Top 5 Orchestrators (Highest 'Imports' / Fragility Index)
These files pull in the most external dependencies. They are highly coupled and fragile to API changes.

1. **galaxyscope.py** (`gitgalaxy/galaxyscope.py`) — 56 outbound dependencies
2. **test_language_standards_strict.py** (`tests/core_engine/test_language_standards_strict.py`) — 29 outbound dependencies
3. **test_dependency_extraction_strict.py** (`tests/extraction/test_dependency_extraction_strict.py`) — 23 outbound dependencies
4. **test_galaxyscope.py** (`tests/core_engine/test_galaxyscope.py`) — 20 outbound dependencies
...
- `draw_ascii_histogram` (@ `gitgalaxy/tools/terabyte_log_scanning/pii_leak_hunter.py`) -> Impact: **480.6** | LOC: 155
...
- `main` (@ `gitgalaxy/tools/supply_chain_security/supply_chain_firewall.py`) -> DB Complexity: **26**

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `tests/core_engine` | 17 | 4818.5 | 2.57% | 0.0% |
| `gitgalaxy` | 6 | 4647.7 | 10.65% | 0.0% |
| `gitgalaxy/core` | 9 | 4163.3 | 14.4% | 3.9% |
| `gitgalaxy/metrics` | 5 | 3149.06 | 16.84% | 4.54% |
| `gitgalaxy/standards` | 8 | 2054.88 | 8.06% | 17.92% |
| `gitgalaxy/recorders` | 8 | 1548.92 | 21.1% | 1.81% |
| `tests/security_auditing` | 16 | 1518.02 | 1.92% | 0.0% |
...
- `gitgalaxy/core/prism.py` -> **16.6356%** Exposure
...
- **Unknown Dependencies:** `822` packages imported that bypass the Zero-Trust whitelist.

## 11. CUMULATIVE RISK HITLIST (Top 10 Highest Risk Files)
> Cumulative Risk is the sum of all individual risk exposures. These files represent the highest multi-dimensional technical debt and architectural fragility.

### 1. `gitgalaxy/core/spatial_mapper.py` (PYTHON) -> Cumulative Risk: **752.6**
...
### 2. `gitgalaxy/metrics/statistical_auditor.py` (PYTHON) -> Cumulative Risk: **686.76**
...
### 3. `gitgalaxy/galaxyscope.py` (PYTHON) -> Cumulative Risk: **677.79**
...
### 4. `gitgalaxy/core/prism.py` (PYTHON) -> Cumulative Risk: **668.21**
- **Archetype:** `file_cluster_16` (Distance: 12.0 IQR)
- **Magnitude:** 847.98 | **LOC:** 729 | **CtrlFlow:** 61.9% | **Authorship Centralization:** 62.5%
- **Primary Risk Drivers:** Spec Match (100.0%), Algorithmic Dos (100.0%), Logic Bomb (100.0%), State Flux (99.9766%)
- **Heaviest Functions:** `_compile_regex_matrix` (Impact: 383.2), `_strip_nested_comments` (Impact: 125.8), `split_streams` (Impact: 125.2)

### 5. `gitgalaxy/cobol_refractor_controller.py` (PYTHON) -> Cumulative Risk: **664.55**
...
### 6. `gitgalaxy/tools/cobol_to_cobol/cobol_compiler_forge.py` (PYTHON) -> Cumulative Risk: **662.72**
...
### 7. `gitgalaxy/standards/config_resolver.py` (PYTHON) -> Cumulative Risk: **654.88**
- **Archetype:** `Unknown Archetype` (Distance: N/A IQR)
...
### 8. `gitgalaxy/metrics/chronometer.py` (PYTHON) -> Cumulative Risk: **648.08**
- **Archetype:** `file_cluster_13` (Distance: 11.689 IQR)
- **Magnitude:** 638.28 | **LOC:** 458 | **CtrlFlow:** 61.8% | **Authorship Centralization:** 60.0%
...
### 9. `gitgalaxy/recorders/sbom_recorder.py` (PYTHON) -> Cumulative Risk: **644.6**
...
### 10. `gitgalaxy/recorders/sarif_recorder.py` (PYTHON) -> Cumulative Risk: **638.6**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 8.402
  * `Choke Point (Betweenness):` 0.003297 | `Ripple Effect (Closeness):` 0.018519
  * `Imports (Out-Degree: 30):` gitgalaxy.security.dependency_audit_cache, gitgalaxy.recorders.sbom_recorder, gitgalaxy.tools.ai_guardrails.ai_appsec_sensor, gitgalaxy.security.manifest_parser, multiprocessing, gitgalaxy.tools.supply_chain_security.supply_chain_firewall, importlib.util, tempfile...
  * `Imported By (In-Degree: 3):` (Excluded from Brief to save tokens)

### `tests/core_engine/test_language_standards_strict.py` (PYTHON | Tier 2 | AI Safe: 0.0%)
- **Global Archetype:** `Unknown Archetype` (Drift: N/A IQR)
- **Magnitude:** 2243.9 | **LOC:** 6744 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_cpp_macro_multiline_spiral` (Impact: 263.1 | O(N^5) | DB: 19)
  * `test_livecode_ambiguity_io_vs_ipc_rpc_br` (Impact: 249.5 | O(N^3) | DB: 2)
  * `test_zig_at_builtins_leading_boundary_re` (Impact: 156.2 | O(N^3) | DB: 13)
  * `test_perl_ambiguity_sweep_shared_literal` (Impact: 148.4 | O(N^3) | DB: 1)
  * `test_php_ambiguity_sweep_shared_literals` (Impact: 143.3 | O(N^3) | DB: 1)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* None
* *Risk/State:* None
* *Architecture:* None
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 0):` shapes, pytest, Reactive.Banana, multiprocessing, Foundation, and, kotlin.collections.List, static...
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 10.732
  * `Choke Point (Betweenness):` 7e-05 | `Ripple Effect (Closeness):` 0.030247
  * `Imports (Out-Degree: 1):` collections, math, gitgalaxy.standards.analysis_lens, networkx, pathlib, token, warnings, typing...
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 10.744
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.035915
  * `Imports (Out-Degree: 0):` os, re, math, gitgalaxy.standards, typing, statistics, logging
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 8.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.01929
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` re, math, gitgalaxy.standards.language_standards, gitgalaxy.standards.gitgalaxy_config, pathlib, typing, time, logging
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` java.util., pytest, Foundation, UIKit, Control.Monad, static, formatting, os...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Top Global Matches:** file_cluster_8: 11.108, file_cluster_16: 11.413, file_cluster_7: 11.507
- **Magnitude:** 862.44 | **LOC:** 2335 | **CtrlFlow:** 73.5% | **Authorship Centralization:** 62.5%
- **Algorithmic:** O(N^6) | **DB Complexity:** 3
- **Risk Profile:** Cognitive Load (13.2889%), Tech Debt (8.0938%)
**Top Internal Functions/Classes:**
...
* *Sec High Risk Execution:* 1 instances
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 414`, `structural_boundaries: 149`, `args: 21`, `func_start: 20`, `class_start: 6`
* *Risk/State:* `safety_bypasses: 51`, `state_mutation: 86`, `dead_code: 1`, `planned_debt: 1`
* *Architecture:* `api: 14`, `concurrency: 1`, `import: 12`
* *Defense:* `safety: 39`, `doc: 65`, `test: 2`
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 6.07
  * `Choke Point (Betweenness):` 0.000166 | `Ripple Effect (Closeness):` 0.024691
  * `Imports (Out-Degree: 3):` collections, re, math, gitgalaxy.core.spatial_correlation, tiktoken, gitgalaxy.standards.analysis_lens, gitgalaxy.standards.language_standards, typing...
  * `Imported By (In-Degree: 3):` (Excluded from Brief to save tokens)

### `gitgalaxy/core/prism.py` (PYTHON | Tier 1.5 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_16` (Drift: 12.0 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 5.138 IQR)
- **Top Global Matches:** file_cluster_16: 12.0, file_cluster_8: 12.061, file_cluster_13: 12.254
- **Magnitude:** 847.98 | **LOC:** 729 | **CtrlFlow:** 61.9% | **Authorship Centralization:** 62.5%
- **Algorithmic:** O(N^6) | **DB Complexity:** 10
- **Risk Profile:** Cognitive Load (29.8245%), Tech Debt (16.6356%)
**Top Internal Functions/Classes:**
  * `_compile_regex_matrix` (Impact: 383.2 | O(N^6) | DB: 10)
    * *Intent:* # 4. GENERIC STRIPPER pattern = self.REGEX_MATRIX.get(family) if not pattern: # Restore mask tokens ...
  * `_strip_nested_comments` (Impact: 125.8 | O(N^4) | DB: 8)
  ...
  * `_strip_segment_comments` (Impact: 59.4 | O(N^4) | DB: 7)
  ...
  * `_strip_single_line_comments` (Impact: 26.7 | O(N^4) | DB: 3)
    * *Intent:* # 1. Legacy Column-1 or Column-7 anchors (Fixed Form)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 120`, `structural_boundaries: 74`, `args: 24`, `func_start: 20`, `class_start: 3`
* *Risk/State:* `safety_bypasses: 21`, `state_mutation: 106`, `fragile_debt: 2`
* *Architecture:* `api: 12`, `import: 4`
* *Defense:* `safety: 4`, `doc: 34`
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 6.369
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 8.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.01929
  * `Imports (Out-Degree: 0):` collections, gitgalaxy.standards, heapq, pathlib, typing, sqlite3, statistics, json...
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` tempfile, or, os, yaml, gitgalaxy.standards.analysis_lens, subprocess, gitgalaxy.galaxyscope, concurrent.futures...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` os, re, pathlib, typing, json, logging
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `gitgalaxy/metrics/chronometer.py` (PYTHON | Tier 2 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_13` (Drift: 11.689 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 5.075 IQR)
- **Top Global Matches:** file_cluster_13: 11.689, file_cluster_8: 11.69, file_cluster_16: 11.934
- **Magnitude:** 638.28 | **LOC:** 458 | **CtrlFlow:** 61.8% | **Authorship Centralization:** 60.0%
...
* *Sec High Risk Execution:* 1 instances
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 63`, `structural_boundaries: 39`, `args: 8`, `func_start: 8`, `class_start: 1`
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 8.043
  * `Choke Point (Betweenness):` 3.8e-05 | `Ripple Effect (Closeness):` 0.01929
  * `Imports (Out-Degree: 1):` os, shutil, gitgalaxy.standards.config_resolver, gitgalaxy.standards, subprocess, pathlib, typing, time...
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` collections, gitgalaxy.standards.analysis_lens, networkx, numpy, pathlib, xgboost, typing, pandas...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 7.804
  * `Choke Point (Betweenness):` 0.000345 | `Ripple Effect (Closeness):` 0.006173
  * `Imports (Out-Degree: 9):` datetime, gitgalaxy.licensing, argparse, gitgalaxy.tools.cobol_to_cobol.cobol_microservice_slicer, gitgalaxy.tools.cobol_to_cobol.cobol_graveyard_finder, pathlib, gitgalaxy.tools.cobol_to_cobol.cobol_jcl_forge, gitgalaxy.tools.cobol_to_cobol.cobol_schema_forge...
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)

### `tests/core_engine/test_detector.py` (PYTHON | Tier 1.5 | AI Safe: 0.0%)
- **Global Archetype:** `Unknown Archetype` (Drift: N/A IQR)
- **Magnitude:** 469.24 | **LOC:** 2024 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_detector_exact_loc_mapping` (Impact: 39.0 | O(N^2))
    * *Intent:* # Put 200 lines of safe padding between them to exceed the 150-char blast radius
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` re, pytest, math, gitgalaxy.standards.language_standards, with, gitgalaxy.standards.gitgalaxy_config, gitgalaxy.core.spatial_mapper, unittest.mock...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` ast, argparse, pathlib, sys, typing, json
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` dataclasses, gitgalaxy.standards, yaml, typing, copy, __future__, logging
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `gitgalaxy/standards/language_standards.py` (PYTHON | Tier 1.5 | AI Safe: 0.0%)
- **Global Archetype:** `Unknown Archetype` (Drift: N/A IQR)
- **Magnitude:** 352.98 | **LOC:** 11245 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* None
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` re, java.util., inside, path, type, typing, keyword.
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `tests/core_engine/test_signal_processor.py` (PYTHON | Tier 1.5 | AI Safe: 0.0%)
- **Global Archetype:** `Unknown Archetype` (Drift: N/A IQR)
- **Magnitude:** 342.34 | **LOC:** 1871 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_signal_processor_llm_api_secrets` (Impact: 74.2 | O(N^4) | DB: 14)
  ...
  * `test_signal_processor_security_lenses` (Impact: 6.9 | O(N^3))
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* None
* *Risk/State:* None
* *Architecture:* None
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` os, identity, pytest, which, gitgalaxy.metrics.signal_processor, tempfile, gitgalaxy.recorders.sarif_recorder, json
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 4.219
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 6):` shutil, gitgalaxy.tools.cobol_to_java.cobol_to_java_api_contract_forge, gitgalaxy.tools.cobol_to_java.cobol_to_java_agent_forge, gitgalaxy.tools.cobol_to_java.cobol_to_java_service_forge, gitgalaxy.licensing, argparse, pathlib, sys...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 9.835
  * `Choke Point (Betweenness):` 0.000307 | `Ripple Effect (Closeness):` 0.024691
  * `Imports (Out-Degree: 4):` os, gitgalaxy.security.manifest_parser, gitgalaxy.security.security_lens, gitgalaxy.standards.language_standards, gitgalaxy.standards.gitgalaxy_config, gitgalaxy.standards.language_lens, pathlib, uuid...
  * `Imported By (In-Degree: 3):` (Excluded from Brief to save tokens)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` gitgalaxy.standards.config_resolver, pytest, was, policy, bypassed, yaml, gitgalaxy.tools.supply_chain_security.supply_chain_firewall, gitgalaxy.metrics.signal_processor...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `gitgalaxy/core/spatial_mapper.py` (PYTHON | Tier 1.5 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_13` (Drift: 11.554 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 5.299 IQR)
- **Top Global Matches:** file_cluster_13: 11.554, file_cluster_16: 11.607, file_cluster_8: 11.698
- **Magnitude:** 257.06 | **LOC:** 230 | **CtrlFlow:** 61.7% | **Authorship Centralization:** 53.8%
- **Algorithmic:** O(N^6) | **DB Complexity:** 9
- **Risk Profile:** Cognitive Load (33.4034%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `map_repository` (Impact: 189.1 | O(N^6) | DB: 7)
...
* *Defense:* `doc: 10`
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 5.174
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.01929
  * `Imports (Out-Degree: 0):` math, typing, hashlib, logging
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)

### `gitgalaxy/core/guidestar_lens.py` (PYTHON | Tier 1.5 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_8` (Drift: 11.088 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 4.941 IQR)
- **Top Global Matches:** file_cluster_8: 11.088, file_cluster_13: 11.23, file_cluster_16: 11.392
- **Magnitude:** 252.3 | **LOC:** 512 | **CtrlFlow:** 62.9% | **Authorship Centralization:** 53.8%
- **Algorithmic:** O(N^6) | **DB Complexity:** 3
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 8.043
  * `Choke Point (Betweenness):` 3.8e-05 | `Ripple Effect (Closeness):` 0.01929
  * `Imports (Out-Degree: 1):` os, fnmatch, re, gitgalaxy.standards.gitgalaxy_config, pathlib, typing, json, logging
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)
...
- `gitgalaxy/metrics/chronometer.py` (PYTHON) | Magnitude: 638.28 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 259, branch: 63, structural_boundaries: 39, state_mutation: 35
...
- `gitgalaxy/core/prism.py` (PYTHON) | Magnitude: 847.98 | Delta: **0.061 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 386, branch: 120, state_mutation: 106, structural_boundaries: 74
...
- `gitgalaxy/recorders/llm_recorder.py` -> Churn: **75.58%** | Cog Load: 62.8023% | Debt: 0.0%
...
- `gitgalaxy/galaxyscope.py` -> **Severity: 0.256** (Bridge: 0.0033 * Flux: 77.6198%)
- `gitgalaxy/cobol_refractor_controller.py` -> **Severity: 0.033** (Bridge: 0.0003 * Flux: 97.0538%)
- `gitgalaxy/recorders/sbom_recorder.py` -> **Severity: 0.03** (Bridge: 0.0003 * Flux: 97.0328%)
- `gitgalaxy/core/detector.py` -> **Severity: 0.008** (Bridge: 0.0002 * Flux: 50.4782%)
- `gitgalaxy/security/security_auditor.py` -> **Severity: 0.008** (Bridge: 0.0001 * Flux: 94.0786%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `gitgalaxy/standards/config_resolver.py` -> **Severity: 5.291** (Embedded: 0.0661 * Error Risk: 80.0%)
- `gitgalaxy/core/network_risk_sensor.py` -> **Severity: 2.202** (Embedded: 0.0302 * Error Risk: 72.8082%)
- `gitgalaxy/recorders/sarif_recorder.py` -> **Severity: 1.559** (Embedded: 0.0247 * Error Risk: 63.1492%)
- `gitgalaxy/tools/ai_guardrails/ai_appsec_sensor.py` -> **Severity: 1.543** (Embedded: 0.0193 * Error Risk: 80.0%)
- `gitgalaxy/recorders/llm_recorder.py` -> **Severity: 1.478** (Embedded: 0.0193 * Error Risk: 76.6083%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `gitgalaxy/standards/config_resolver.py` -> **Severity: 3308.0** (Blast Radius: 33.08 * Doc Risk: 100.0%)
- `gitgalaxy/metrics/tensor_scanner.py` -> **Severity: 804.3** (Blast Radius: 8.043 * Doc Risk: 100.0%)
- `gitgalaxy/standards/language_standards.py` -> **Severity: 787.545** (Blast Radius: 48.419 * Doc Risk: 16.2652%)
- `gitgalaxy/tools/cobol_to_cobol/cobol_compiler_forge.py` -> **Severity: 780.4** (Blast Radius: 7.804 * Doc Risk: 100.0%)
- `gitgalaxy/tools/network_auditing/full_api_network_map.py` -> **Severity: 742.787** (Blast Radius: 8.043 * Doc Risk: 92.352%)
