...
| **Timestamp** | `2026-08-02T20:33:40.076668+00:00` |
| **Scan Duration** | `9.55s` |
| **Git Branch** | `main` |
| **Git Commit** | `9c58446b00fcff0533103598517d9ee5019f9288` |
...
|---|---|
| Total Artifacts | 690 |
| Analyzed Artifacts (Scanned) | 215 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 475 |
| Total LOC | 56089 |
| Volatility Index | 0.014 |
| % Scanned of codebase = | 31.2% |
| Dominant Lang | PYTHON |

## 3.5 MACRO-NETWORK TOPOLOGY (Resilience & Coupling)
| Metric | Value | Interpretation |
|---|---|---|
| Modularity | 0.6399 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | -0.3173 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.0% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 2.8418 | Hops between files. Lower = Tighter coupling. |
| Articulation Pts | 36 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| PYTHON | 171 | 55586 | 79.5% |
| MARKDOWN | 29 | 0 | 13.5% |
| YAML | 8 | 481 | 3.7% |
| PLAINTEXT | 6 | 0 | 2.8% |
| SHELL | 1 | 22 | 0.5% |
...
|---|---|---|
| file_cluster_8 | 121 | 56.3% |
| file_cluster_13 | 32 | 14.9% |
| file_cluster_0 | 19 | 8.8% |
| file_cluster_16 | 7 | 3.3% |
| file_cluster_6 | 1 | 0.5% |

### Inert Structural Mass (Static Categories)
| Category | Count | Repo % |
|---|---|---|
| Static: Literature & Documentation | 35 | 16.3% |
...
| Specification Exposure | 0.0 | 100.0 | 16.4 | 0.0 | 0.0 |
...
1. **language_standards.py** (`gitgalaxy/standards/language_standards.py`) — 52 inbound connections
2. **_extraction_harness.py** (`tests/extraction/_extraction_harness.py`) — 35 inbound connections
...
| `tests/extraction/languages` | 35 | 2505.35 | 2.71% | 0.0% |
...
- **Unknown Dependencies:** `1146` packages imported that bypass the Zero-Trust whitelist.

## 11. CUMULATIVE RISK HITLIST (Top 10 Highest Risk Files)
> Cumulative Risk is the sum of all individual risk exposures. These files represent the highest multi-dimensional technical debt and architectural fragility.

### 1. `gitgalaxy/core/spatial_mapper.py` (PYTHON) -> Cumulative Risk: **748.16**
...
### 2. `gitgalaxy/metrics/statistical_auditor.py` (PYTHON) -> Cumulative Risk: **682.82**
...
### 3. `gitgalaxy/core/prism.py` (PYTHON) -> Cumulative Risk: **669.08**
...
### 4. `gitgalaxy/galaxyscope.py` (PYTHON) -> Cumulative Risk: **668.17**
- **Archetype:** `file_cluster_8` (Distance: 12.513 IQR)
- **Magnitude:** 3862.46 | **LOC:** 3044 | **CtrlFlow:** 71.1% | **Authorship Centralization:** 65.6%
- **Primary Risk Drivers:** Spec Match (100.0%), Algorithmic Dos (100.0%), Logic Bomb (100.0%), Churn (86.62%)
- **Heaviest Functions:** `execute_pipeline` (Impact: 2143.0), `main` (Impact: 869.6), `_process_file_worker` (Impact: 611.9)

### 5. `gitgalaxy/tools/cobol_to_cobol/cobol_compiler_forge.py` (PYTHON) -> Cumulative Risk: **656.5**
...
### 6. `gitgalaxy/cobol_refractor_controller.py` (PYTHON) -> Cumulative Risk: **656.21**
...
### 7. `gitgalaxy/standards/config_resolver.py` (PYTHON) -> Cumulative Risk: **652.98**
- **Archetype:** `Unknown Archetype` (Distance: N/A IQR)
...
### 8. `gitgalaxy/metrics/chronometer.py` (PYTHON) -> Cumulative Risk: **647.05**
...
### 9. `gitgalaxy/recorders/sbom_recorder.py` (PYTHON) -> Cumulative Risk: **639.88**
...
### 10. `gitgalaxy/recorders/sarif_recorder.py` (PYTHON) -> Cumulative Risk: **634.56**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 0):` numpy, Brick, mypackage.MyClass, os.path, still, multiprocessing, x, ccall...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `gitgalaxy/galaxyscope.py` (PYTHON | Tier 2 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_8` (Drift: 12.513 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 4.538 IQR)
- **Top Global Matches:** file_cluster_8: 12.513, file_cluster_13: 12.65, file_cluster_7: 12.899
- **Magnitude:** 3862.46 | **LOC:** 3044 | **CtrlFlow:** 71.1% | **Authorship Centralization:** 65.6%
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 6.214
  * `Choke Point (Betweenness):` 0.001887 | `Ripple Effect (Closeness):` 0.014019
  * `Imports (Out-Degree: 30):` signal, gitgalaxy.security.dependency_audit_cache, gitgalaxy.security.security_lens, DAG, multiprocessing, gitgalaxy.tools.ai_guardrails.ai_appsec_sensor, importlib.util, gitgalaxy.recorders.sbom_recorder...
...
- **Top Global Matches:** file_cluster_13: 11.184, file_cluster_8: 11.248, file_cluster_16: 11.32
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 7.937
  * `Choke Point (Betweenness):` 4e-05 | `Ripple Effect (Closeness):` 0.022897
  * `Imports (Out-Degree: 1):` networkx, logging, pathlib, warnings, collections, token, networkx.algorithms, math...
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 7.946
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.027188
  * `Imports (Out-Degree: 0):` logging, statistics, gitgalaxy.standards, math, typing, os, re
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 5.948
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.014603
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` logging, gitgalaxy.standards.language_standards, pathlib, math, gitgalaxy.standards.gitgalaxy_config, time, typing, re
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.018692
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 4.49
  * `Choke Point (Betweenness):` 9.5e-05 | `Ripple Effect (Closeness):` 0.018692
  * `Imports (Out-Degree: 3):` bisect, gitgalaxy.core.spatial_correlation, logging, gitgalaxy.standards.language_standards, tiktoken, collections, gitgalaxy.standards.analysis_lens, math...
  * `Imported By (In-Degree: 3):` (Excluded from Brief to save tokens)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 5.948
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.014603
  * `Imports (Out-Degree: 0):` logging, statistics, pathlib, heapq, gitgalaxy.standards, collections, sqlite3, typing...
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` name, failure, unittest.mock, concurrent.futures, tempfile, must, passes, or...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` json, logging, pathlib, typing, os, re
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 5.948
  * `Choke Point (Betweenness):` 2.2e-05 | `Ripple Effect (Closeness):` 0.014603
  * `Imports (Out-Degree: 1):` subprocess, logging, pathlib, gitgalaxy.standards.config_resolver, gitgalaxy.standards, shutil, time, typing...
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` gitgalaxy.core.detector, with, logging, gitgalaxy.standards.language_standards, gitgalaxy.core.prism, gitgalaxy.core.spatial_mapper, pytest, unittest.mock...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` networkx, logging, xgboost, numpy, pathlib, collections, pandas, gitgalaxy.standards.analysis_lens...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` ..., gitgalaxy.standards.language_standards, com.example.MyClass, _extraction_harness, statement, pathlib, pytest, only...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 5.772
  * `Choke Point (Betweenness):` 0.000197 | `Ripple Effect (Closeness):` 0.004673
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` argparse, ast, pathlib, typing, sys, json
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` form, scala.util., was, type, scala.util.chaining., keyword., java.util., path...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` logging, yaml, gitgalaxy.standards, copy, dataclasses, __future__, typing
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` identity, tempfile, pytest, which, gitgalaxy.recorders.sarif_recorder, gitgalaxy.metrics.signal_processor, os, json
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Unknown | `Dependency Blast Radius (PageRank):` 0.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` bypassed, was, yaml, gitgalaxy.tools.supply_chain_security.supply_chain_firewall, policy, pathlib, pytest, gitgalaxy.standards.config_resolver...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 3.12
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 6):` gitgalaxy.tools.cobol_to_java.cobol_to_java_spring_forge, gitgalaxy.tools.cobol_to_java.cobol_to_java_decoder_forge, argparse, org.slf4j.Logger, org.springframework.stereotype.Service, org.slf4j.LoggerFactory, pathlib, gitgalaxy.licensing...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Top Global Matches:** file_cluster_13: 10.631, file_cluster_8: 10.717, file_cluster_16: 11.045
- **Magnitude:** 283.32 | **LOC:** 351 | **CtrlFlow:** 51.9% | **Authorship Centralization:** 100.0%
- **Algorithmic:** O(N^6) | **DB Complexity:** 6
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 7.274
  * `Choke Point (Betweenness):` 0.000176 | `Ripple Effect (Closeness):` 0.018692
  * `Imports (Out-Degree: 4):` logging, gitgalaxy.standards.language_standards, gitgalaxy.security.security_lens, gitgalaxy.security.manifest_parser, pathlib, gitgalaxy.standards.language_lens, gitgalaxy.standards.gitgalaxy_config, uuid...
  * `Imported By (In-Degree: 3):` (Excluded from Brief to save tokens)
...
* *Defense:* `doc: 10`
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 3.827
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.014603
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 5.948
  * `Choke Point (Betweenness):` 2.2e-05 | `Ripple Effect (Closeness):` 0.014603
  * `Imports (Out-Degree: 1):` json, logging, pathlib, fnmatch, gitgalaxy.standards.gitgalaxy_config, typing, os, re
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)
...
- `gitgalaxy/galaxyscope.py` -> **Severity: 0.146** (Bridge: 0.0019 * Flux: 77.6198%)
...
- `gitgalaxy/standards/config_resolver.py` -> **Severity: 4.005** (Embedded: 0.0501 * Error Risk: 80.0%)
- `gitgalaxy/core/network_risk_sensor.py` -> **Severity: 1.667** (Embedded: 0.0229 * Error Risk: 72.8082%)
- `gitgalaxy/standards/language_standards.py` -> **Severity: 1.19** (Embedded: 0.2474 * Error Risk: 4.8115%)
- `gitgalaxy/recorders/sarif_recorder.py` -> **Severity: 1.18** (Embedded: 0.0187 * Error Risk: 63.1492%)
- `gitgalaxy/tools/ai_guardrails/ai_appsec_sensor.py` -> **Severity: 1.168** (Embedded: 0.0146 * Error Risk: 80.0%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `gitgalaxy/standards/config_resolver.py` -> **Severity: 2446.6** (Blast Radius: 24.466 * Doc Risk: 100.0%)
- `gitgalaxy/standards/language_standards.py` -> **Severity: 1464.942** (Blast Radius: 84.872 * Doc Risk: 17.2606%)
- `gitgalaxy/metrics/tensor_scanner.py` -> **Severity: 594.8** (Blast Radius: 5.948 * Doc Risk: 100.0%)
- `gitgalaxy/tools/cobol_to_cobol/cobol_compiler_forge.py` -> **Severity: 577.2** (Blast Radius: 5.772 * Doc Risk: 100.0%)
- `gitgalaxy/tools/network_auditing/full_api_network_map.py` -> **Severity: 560.581** (Blast Radius: 5.948 * Doc Risk: 94.247%)
