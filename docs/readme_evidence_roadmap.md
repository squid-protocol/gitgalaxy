...
| Fitted two-regime scan-speed model, replacing the "a handful of 20M+ LOC outliers take a few minutes" hand-wave: flat ~0.11s below ~4,258 LOC (fixed overhead), `time(s) ≈ 3.36e-05 × LOC^0.969` above it (R²=0.88, 599 repos) — both the knee and the exponent are derived from `gitgalaxy-raw-output`'s own batch data (`tools/generate_speed_telemetry.py`'s `compute_rate_model()`), not eyeballed. LOC-vs-time chart embedded from that repo's self-updating `speed_charts/latest/loc_vs_time.png`, so it never goes stale here | 2026-08-06 | README.md "What Pain Point Does This Solve?" (speed bullet) and "Proof, Not Just Claims" (item 3, plus the embedded chart) |
| Measured structural-extraction accuracy vs. Tree-sitter ground truth: median ~98% function recall / ~96% precision across 31 baselined languages, with the real range (Kotlin at 39.1% recall) stated alongside the median rather than hidden by it. Backed by `tests/tools/tree_sitter_accuracy_audit.py`, one committed baseline per language, re-measured and re-charted automatically on every push touching the parsing engine — the same "always current, embedded from the repo" pattern as the speed chart above, not a one-time number. `python.md` and `javascript.md` under `docs/language_status/` have full written methodology and found real defects along the way | 2026-08-12 | README.md "Proof, Not Just Claims" (new item 4, plus the embedded `docs/self_scan/tree_sitter_accuracy_chart.svg`), and "One Graph, Not Five Separate Tools" (the "why not Tree-sitter" paragraph, which now links to this measurement instead of asserting the tradeoff unbacked) |
