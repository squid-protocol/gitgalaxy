...
| `calls_out` | structure | `site` | declared |  | One entry is one distinct callable unit, by name, that the function's own body invokes -- a call, a constructor or conversion, a macro, or a verb-form invocation (CALL, PERFORM, EXEC PGM=, a command word) -- other than the function itself | [calls_out_rule_contract.md](../docs/calls_out_rule_contract.md) #3327 |
...
| `auth_middleware` | hybrid | `site` | declared |  | A site that performs or gates authentication or privilege -- a sign-on or sign-off, a credential verification, a permission or security query, a privilege GRANT or REVOKE, or the registration of an auth middleware or filter -- at its invocation | [domain_sensor_contracts.md](../docs/domain_sensor_contracts.md) #3004 |
...
| `system_config_mutation` | hybrid | `site` | declared |  | A site that durably mutates shared infrastructure or subsystem configuration -- a resource-definition update against a system catalog or control store, a persistent system-parameter, registry or service-configuration write -- in the mutating utility's or primitive's own invocation or statement form | [domain_sensor_contracts.md](../docs/domain_sensor_contracts.md) #3084 |
