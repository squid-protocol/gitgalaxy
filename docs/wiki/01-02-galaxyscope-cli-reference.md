...
* **`--ai-guardrails`**
  * **Action:** Enables Phase 5 (Zero-Trust AI Guardrails — `dev_agent_firewall.py` + `ai_appsec_sensor.py`). The phase is **off by default**; without the flag the guardrail telemetry columns (`agentic_isolation_risk`, `requires_hitl`, `hallucination_zone`, `silent_mutation_risk`, `appsec_god_mode`) record `NULL` ("not evaluated" — `0` is reserved for "evaluated, no risk found") and no `GG-AGENT-GUARDRAIL` SARIF results are emitted. Also settable as `ai-guardrails: true` under the `galaxyscope:` section of `.galaxyscope.yaml`.
  * **Use Case:** Repositories with a real AI/agentic surface (LLM orchestration frameworks, autonomous coding agents). Most targets — e.g. a large COBOL modernization scan — have none, which is why the phase no longer pays its full per-file cost on every scan. Enabling is always an explicit choice; the engine never auto-detects, so turning the phase on is visible in the CLI/config rather than dependent on a heuristic.
