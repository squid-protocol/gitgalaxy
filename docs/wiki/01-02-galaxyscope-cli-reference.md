...
* **`--ai-guardrails`**
  * **Action:** Enables Phase 5 (Zero-Trust AI Guardrails — `dev_agent_firewall.py` + `ai_appsec_sensor.py`). The phase is **off by default**; without the flag the guardrail telemetry columns (`agentic_isolation_risk`, `requires_hitl`, `hallucination_zone`, `silent_mutation_risk`, `appsec_god_mode`) record their 0/false defaults and no `GG-AGENT-GUARDRAIL` SARIF results are emitted. Also settable as `ai-guardrails: true` under the `galaxyscope:` section of `.galaxyscope.yaml`.
