...
* **CICS resource operations (#3351-#3354)** ride the cobol and pli dialects as `cics_resources` (`core/cics_resources.py`): one row per `EXEC CICS` command that names a resource -- FILE I/O (READ/WRITE/REWRITE/DELETE/browse), SEND/RECEIVE MAP (+ MAPSET), WRITEQ/READQ/DELETEQ TS|TD, PUT/GET/MOVE/DELETE CONTAINER (+ CHANNEL), and the CHANNEL a LINK/XCTL/START/RETURN/RUN hands on -- with its access direction, the name resolved through its VALUE (as LINK targets are) or a single MOVEd literal, and the INTO/FROM record. One table for all five kinds, persisted as `cics_resource_data`; joining a map to its BMS fields and producers to consumers is `galaxy_ir`'s job.
