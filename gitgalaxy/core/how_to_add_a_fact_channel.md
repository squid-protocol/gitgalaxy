...
| IMS DL/I calls: EXEC DLI and CALL 'CBLTDLI' with operands as written (function codes and SSAs resolved through COPY-expanded VALUEs, and the program x segment matrix, in the reader; PSB / DBD sources not yet scanned) | #3450 | `core/dli_calls.py` (via `extract_boundary`, cobol) | `dli_call_data` | `EngineFile.dli_calls` (`GalaxyIR.ims_calls`/`ims_segment_access`) |
