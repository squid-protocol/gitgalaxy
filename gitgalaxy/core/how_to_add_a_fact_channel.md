...
| CICS resource operations: FILE / MAP / QUEUE / CONTAINER / CHANNEL (one table for four issues; cross-file joins in the reader) | #3351-#3354 | `core/cics_resources.py` (via `extract_boundary`, cobol + pli + hlasm) | `cics_resource_data` | `EngineFile.cics_resources` (`GalaxyIR.screen_bindings`/`queue_flows`/`container_flows`) |
...
| CICS task control: RUN/START children (a STRING-built transid kept as a PIC-sized fnmatch pattern), FETCH/FREE joins, RETRIEVE, DELAY, POST, WAIT, ENQ/DEQ (children joined to the CSD in the reader) | #3449 | `core/cics_tasks.py` (via `extract_boundary`, cobol + pli + hlasm) | `cics_task_data` | `EngineFile.cics_tasks` (`GalaxyIR.async_tasks`) |
...
| units of work and error handling: SYNCPOINT / SQL COMMIT / ROLLBACK, HANDLE CONDITION / ABEND / AID, explicit ABEND, RESP checks tied to their command (owning paragraph and TD trigger starts in the reader) | #3453 | `core/uow_handlers.py` (via `extract_boundary`, cobol + hlasm + pli; PL/I ON / REVERT / SIGNAL rows from `core/pli_on_units.py`, #3491) | `uow_handler_data` | `EngineFile.uow_handlers` (`GalaxyIR.units_of_work`/`error_handlers`/`unchecked_responses`/`tdq_trigger_starts`) |
...
| web / API services: the CICS web-services assistant steps (DFHLS2WS / DFHLS2JS providers, DFHWS2LS / DFHJS2LS requesters) with PGMNAME / URI / REQMEM / RESPMEM / PGMINT / WSBIND / WSDL from their in-stream parameters; program + copybook joins and the CSD URIMAP / PIPELINE / WEBSERVICE / TCPIPSERVICE definitions in the reader (program-side WEB / INVOKE / TRANSFORM are `cics_resources` rows, #3512; the reader joins an INVOKE SERVICE to its requester step and lists hand-coded HTTP providers / clients) | #3496 | `core/web_services.py` (via `extract_boundary`, jcl) | `web_service_data` | `EngineFile.web_services` (`GalaxyIR.api_surface`; completeness counts exposed programs reached) |
| JCICS (Java on CICS): `Program.link()` as LINK call sites (joining Java to the COBOL call graph), KSDS / ESDS / RRDS / TSQ / TDQ operations, createChannel / create- / getContainer, names from setName literals / constants / prefixes | #3497 | `core/jcics.py` (via `extract_boundary`, java) | `call_site_data` + `cics_resource_data` (attributes `JCICS <type>`) | `EngineFile.calls` / `cics_resources` (the COBOL shapes; resolver + completeness see them) |
| command-level `EXEC CICS` in HLASM (a second host on the existing CICS channels): each assembler statement, continued at column 72, is rewritten into the END-EXEC shape the COBOL walkers read, line-aligned; operands resolve through `DC C'...'` constants; `OC v,v` / `CLC v,=F'n'` count as RESP checks; `name DFHEIENT` opens the program unit | #3495 | `core/hlasm_cics.py` (via `extract_boundary`, hlasm) | `call_site_data` + `cics_resource_data` + `cics_task_data` + `uow_handler_data` | `EngineFile.calls` / `cics_resources` / `cics_tasks` / `uow_handlers` (the COBOL shapes) |
| PL/I program call sites (a second host on the call-site channel): EXEC CICS LINK / XCTL / RETURN\|START\|RUN TRANSID through the COBOL reader (each command closed by END-EXEC before its `;`, sequence fields blanked by shape), and `CALL` to an entry outside the compilation unit; the resolver indexes a PL/I program by member (file stem) and outermost procedure label, and drops a CALL to a name that is only ever a nested procedure elsewhere (an %INCLUDE member calling its includer) | #3491 | `core/pli_calls.py` (via `extract_boundary`, pli) + `core/invocation_resolver.py` | `call_site_data` | `EngineFile.calls` (the COBOL shape; resolver + completeness see them) |
