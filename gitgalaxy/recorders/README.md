...
### `cics_resource_data` — CICS resource operations (#3351-#3354)

One row per `EXEC CICS` command in a COBOL/PL/I source that names a resource. `resource_kind` is `FILE` (READ/READNEXT/READPREV/STARTBR/RESETBR/ENDBR/WRITE/REWRITE/DELETE/UNLOCK with FILE or DATASET), `MAP` (SEND/RECEIVE MAP), `QUEUE` (WRITEQ/READQ/DELETEQ with QUEUE or QNAME), `CONTAINER` (PUT/GET/MOVE/DELETE CONTAINER) or `CHANNEL` (a CHANNEL handed on by LINK/XCTL/START/RETURN/RUN); `verb` is the command and `access` its direction (`read`/`write`/`update`/`delete`/`browse`/`unlock`/`move`/`pass` -- `write` produces, `read` consumes). `name_operand` is the name as written and `resource_name` its value: the literal, the data-name's working-storage `VALUE` (same file, as LINK targets resolve), or the one literal ever `MOVE`d to it; `name_resolution` says which (`literal`/`value`/`move`), or why there is none (`ambiguous` -- the MOVEd literals are in `name_candidates` -- `unresolved`, `expression`). `qualifier_operand`/`qualifier` carry a MAP's MAPSET, a CONTAINER's CHANNEL and a CHANNEL's receiving PROGRAM/TRANSID (as written / resolved); for a QUEUE `qualifier` is `TS` or `TD`. `record_clause`/`record_name` are the INTO/FROM/SET data area, `attributes` every other option (RESP/RESP2/NOHANDLE dropped) and `line_number` the `EXEC CICS` line. One table rather than four: every kind is a verb, a direction, one name, one qualifier and one record, and consumers ask all of them the same producer/consumer question. Per-file, cascade-deleted with `file_data`, restored on delta scans.
