...
"[TARGET LANGUAGE]": {
    ...
    "rules": {
        ...
        # calls_out: One entry is one distinct callable unit, by name, that the function's own body invokes -- a call, a constructor or conversion, a macro, or a verb-form invocation (CALL, PERFORM, EXEC PGM=, a command word) -- other than the function itself. Keywords are never calls; built-ins are (the resolver labels them external). A name passed as a value, a metadata annotation, a nested declaration and a goto are not calls. Bare name only (the qualifier is a separate datum); deduplicated; no self-recursion; runs after the literal shield. Languages with no by-name invocation form use CALLS_OUT_UNSUPPORTED. Full contract, decisions and the 65-language audit: docs/calls_out_rule_contract.md (#3327).
        "calls_out": re.compile(r""), 
