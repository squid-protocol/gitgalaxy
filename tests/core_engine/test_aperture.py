...
def test_aperture_sql_ddl_exempt_from_machine_gen_shield(tmp_path):
    ...
    comment as noise -- true for compiled/transpiled output, false for SQL DDL/DML,
    where "generated by <tool>" conventionally means a canonical, committed migration
    (MediaWiki's abstractSchemaChanges, Prisma migrations), not vendor noise. Gate 3.3
    (Static Asset Bloat Deflector) already runs its own size-tiered noise gate for this
    exact extension set, so this exemption doesn't leave SQL ungated entirely.

    Uses the REAL LANGUAGE_DEFINITIONS/APERTURE_CONFIG (not the file's minimal
    MOCK_REGISTRY, which has no "sql"/"sqlite" entry and would reject the file at the
    unrelated Gate 1.5 unsupported-extension check before ever reaching Gate 4.3).
    """
    from gitgalaxy.standards.gitgalaxy_config import APERTURE_CONFIG
    from gitgalaxy.standards.language_standards import LANGUAGE_DEFINITIONS
    ...
    sql_file = tmp_path / "patch-example.sql"
    sql_content = (
        "-- This file is automatically generated using maintenance/generateSchemaChangeSql.php.\n"
        "CREATE TABLE /*_*/example (id INTEGER PRIMARY KEY);\n"
    )
    sql_file.write_text(sql_content, encoding="utf-8")
    result = real_engine.is_in_scope(sql_file, content=sql_content)
    assert result["is_in_scope"] is True
    ...
    result = real_engine.is_in_scope(py_file, content=py_content)
    assert result["is_in_scope"] is False
    assert "Machine-Generated Source Code Signature" in result["reason"]


def test_aperture_sql_ddl_exempt_from_micro_mass_quota(tmp_path):
    """
    #2512: the Neighborhood Micro-Mass Quota (galaxyscope.py) already exempted COBOL/JCL
    for being legitimately tiny AND legitimately numerous in one directory -- confirmed
    the same is true of SQL DDL/DML via Prisma's typed-SQL convention (one bare query
    per file, language-crucible's prisma_typed_sql/ has 16 real files at 42-49 bytes
    each in one directory), which silently dropped the 16th past the 15-file grace
    limit. This test exercises the Orchestrator directly (not just ApertureFilter)
    since the quota lives in galaxyscope.py's file-census phase, not aperture.py.
    """
    from gitgalaxy.galaxyscope import Orchestrator

    scope = Orchestrator(str(tmp_path), config={})
    assert ".sql" in scope.MICRO_MASS_EXEMPT_EXTENSIONS
    assert ".ddl" in scope.MICRO_MASS_EXEMPT_EXTENSIONS
