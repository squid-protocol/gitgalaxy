...
class TestGalaxyScopeOrchestrator(unittest.TestCase):
    ...
    def test_phantom_file_race_condition(self, mock_is_file):
        ...
        self.assertEqual(
            result["reason"], "Phantom file (missing on disk)", "Worker returned incorrect phantom reason!"
        )
        ...
        mock_repository_graph = [
            {
                ...
                "risk_vector": [
                    ...
                    0.0,
                    100.0,
                ],  # Index doesn't matter, max is 100.0 (Triggers Max Risk Gate)
                "telemetry": {
    ...
    def test_yaml_configuration_and_cli_priority(self, mock_license, mock_orchestrator):
        ...
        test_args = ["galaxyscope", ".", "--config", temp_yaml_path, "--max-risk-exposure", "80.0"]
        ...
        try:
            ...
            self.assertEqual(
                ignited_config["MAX_RISK_EXPOSURE"], 80.0, "YAML illegally overwrote an explicit CLI flag!"
            )
    ...
    def test_explicit_cli_sentinel_value_not_clobbered_by_yaml(self, mock_license, mock_orchestrator):
        ...
        try:
            ...
            self.assertEqual(
                ignited_config["MAX_RISK_EXPOSURE"],
                0.0,
    ...
    def test_yaml_can_set_flag_with_previously_truthy_default(self, mock_license, mock_orchestrator):
        ...
        try:
            ...
            self.assertEqual(
                ignited_config["DEPENDENCY_SCAN_BUDGET"],
                5,
    ...
    def test_incremental_delta_shift(self, mock_db, mock_calc, mock_extract):
        ...
        def mock_extract_action():
            ...
            self.assertNotIn(
                "src/untouched.py", scope.stem_map, "Delta scan wastefully targeted an untouched file during Phase 1!"
            )
        ...
        scope.execute_incremental_scan(
            ram_cache=old_ram_cache, added=added, modified=modified, deleted=deleted, db_output_path="fake_db.sqlite"
        )
    ...
    def test_sarif_sanitization_purge(self, mock_sarif, mock_db, mock_calc, mock_res, mock_ext, mock_cen):
        ...
        self.assertNotIn(
            ...
            sanitized_file["telemetry"]["threat_snippets"],
            "Failed to purge suppressed snippet!",
        )
    ...
    @patch("gitgalaxy.galaxyscope.RecordKeeper")
    @patch("gitgalaxy.galaxyscope.SarifRecorder")
    def test_repo_z_score_backfilled_into_per_file_telemetry(
        self, mock_sarif, mock_db, mock_calc, mock_res, mock_ext, mock_cen
    ):
        """
    ...
    def test_synthetic_node_generation(self, mock_audit_model):
        ...
        mock_audit_model.return_value = {"architecture": "Llama", "parameters": "8B", "quantization": "Q4_K_M"}
    ...
    def test_instructional_density_and_test_umbrella(self, mock_test_map):
        ...
        self.assertEqual(
            doc_node["metadata"]["doc_umbrella"], 0.9, "Instructional Density failed to multiply the doc shield!"
        )
    ...
    def test_telemetry_rendering_charts(self, mock_print):
        ...
        scope.file_speed_telemetry = {"phase_totals": {"1_Aperture_Filter": 5.0, "2_Disk_IO": 10.0}, "file_count": 1000}
        scope.splicing_telemetry = {
            "top_slowest": [{"path": "src/slow.py", "time": 15.0}, {"path": "src/fast.py", "time": 0.1}],
    ...
    def test_malformed_extension_sanitization(self):
        ...
        self.assertEqual(
            comp["no_extension"]["Test Reason"], 3, "Failed to aggregate all malformed extensions together!"
        )
    ...
    def test_worker_token_extraction_crash_survivability(self, mock_is_file, MockAperture):
        ...
        config["LANGUAGE_DEFINITIONS"] = {
            "python": {
                "extensions": [".py"],
                "rules": {"_dependency_capture": mock_regex, "_named_token_capture": mock_regex},
            }
        }
