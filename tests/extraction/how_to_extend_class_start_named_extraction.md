...
kotlin, zig, rust, haskell, and dart are done** (flipped into the allowlist, see the table below);
5 remain. (c is also done as of a concurrent PR -- #1295's own comment thread has the current count
if this and that PR merge in an unexpected order.)
...
**ruby** (9). (c is also done as of a concurrent PR.)
(**swift, kotlin, zig, rust, haskell** done -- see the "Done" table above.)
