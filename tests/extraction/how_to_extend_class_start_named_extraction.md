...
kotlin, zig, rust, haskell, c, and ruby are done** (flipped into the allowlist, see the table
below); 4 remain (dart is also done as of a concurrent PR -- #1295's own comment thread has the
current count once both land).
...
(**swift, kotlin, zig, rust, haskell, c, ruby** done -- see the "Done" table above; dart is also
done as of a concurrent PR.)
