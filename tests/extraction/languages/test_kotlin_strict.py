...
_KOTLIN_SIMPLE_CASES = [
    ...
    ("func_start", "fun com.example.MyClass.extension(x: Int) {", "val a = 4"),
    ("class_start", "public final data class User(val name: String)", "val klass = classOf()"),
    ("class_start", "internal sealed interface ASTNode {", "fun classify() {"),
    ("class_start", "companion object Factory {", "object : Runnable {"),
    ("class_start", "companion object {", "val myObj = object { }"),
    ("class_start", "enum class Color {", "val a = 1"),
    ("class_start", "value class Password(val s: String)", "val b = 2"),
    ("class_start", "@Serializable\nprivate actual inner class `weird class`", "val c = 3"),
    ("structural_boundaries", "package com.example", "val structural = 1"),
    ("structural_boundaries", "import java.util.*", "val pkg = 1"),
