...
_POWERSHELL_SIMPLE_CASES = [
    ...
    ("branch", "if ($x) {", "$breakpointHandler = $null"),
    ("args", "function foo($x) {", "if ($x -eq $y) {"),
    ("structural_boundaries", "function foo {", "$processId = 123"),
    ("func_start", "function Foo {", "if ($x) {"),
    ("class_start", "class Foo {", "# class Foo (deprecated)"),
    ("safety", "try { risky() } catch {}", "-ErrorAction SilentlyContinue"),
    ("safety_bypasses", "Out-Null", "-ErrorAction Stop"),
    ("high_risk_execution", "Invoke-Expression $cmd", "Invoke-Command -ScriptBlock {}"),
    ("io", "Get-Content $path", "Get-ChildItem $path"),
    ("api", "Export-ModuleMember -Function Foo", "hidden Foo() { }"),
    ("state_mutation", "$x = 5", "$x -eq 5"),
    ("dead_code", "# function Foo {}", "# just a note"),
    ("doc", ".SYNOPSIS", "$PSCmdlet.ParameterSetName"),
    ("test", "Should -Be 5", "# It works correctly in most cases"),
    ("concurrency", "Start-Job { Get-Process }", "Start-Process notepad.exe"),
    ("ui_framework", "Out-GridView", "[System.Windows.Media.Colors]::Blue"),
    ("closures", "{ $_.Name }", "if ($x) {"),
    ("globals", "$global:x = 1", "$local:x = 1"),
    ("decorators", "[Parameter(Mandatory=$true)]", "[ValidateNotNull()]"),
    ("generics", "[List[int]]::new()", "[int]$x"),
    ("comprehensions", "$x | Where-Object { $_ -gt 5 }", "$x | Sort-Object"),
    ("scientific", "[Math]::Sqrt(4)", "[Math]::E"),
    ("reflection_metaprogramming", "Add-Type -TypeDefinition $src", "Add-Member -MemberType NoteProperty"),
    ("import", "Import-Module Foo", "Update-Module Foo"),
    ("ownership", "# Author: Jane Doe", "# Reviewed by: Jane Doe"),
    ("planned_debt", "# TODO: refactor", "# See our TODOS backlog for details"),
    ("fragile_debt", "# HACK: workaround", "# This code is hacky in places"),
    ("spec_exposure", "[SPEC-123]", "[specification]"),
    ("ssr_boundaries", "New-PodeServer", "New-Object System.Net.WebClient"),
    ("events", "Register-ObjectEvent $obj EventName", "Register-ArgumentCompleter -CommandName Foo"),
    ("dependency_injection", "Get-Service Foo", "Get-Process Foo"),
    ("pointers", "[IntPtr]::Zero", "[int]$x"),
    (
        "memory_alloc",
        "[System.Runtime.InteropServices.Marshal]::AllocHGlobal(10)",
        "[System.Runtime.InteropServices.Marshal]::FreeHGlobal($ptr)",
    ),
    ("telemetry", "Write-Verbose 'msg'", "Write-Output 'msg'"),
    ("debug_prints", "Write-Host 'msg'", "Write-Output 'msg'"),
    ("explicit_casts", "[int]$x", "[IntPtr]::Zero"),
    ("panics_and_aborts", "throw 'error'", "-ErrorAction SilentlyContinue"),
    ("thread_sleeps", "Start-Sleep 5", "Start-Process notepad.exe"),
    ("bitwise_ops", "$a -band $b", "$x = -not $flag"),
    ("sync_locks", "[System.Threading.Monitor]::Enter($lock)", "[System.Threading.Interlocked]::Increment($counter)"),
    ("immutability_locks", "New-Variable Foo -Option Constant", "New-Variable Foo -Value 5"),
    ("cleanup", "Remove-Item $path", "$items.Remove($x)"),
    ("encapsulation", "hidden [int] $x", "$isHiddenFlag = $true"),
    ("listeners", "Register-ObjectEvent $obj EventName", "Register-EngineEvent -SourceIdentifier Foo"),
    ("test_skip", "It 'test' -Skip", "$skippedTests = 0"),
    ("serialization_parsing", "ConvertFrom-Json $str", "Export-Clixml -Path out.xml -InputObject $obj"),
