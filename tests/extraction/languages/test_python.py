...
PYTHON_RULES = LANGUAGE_DEFINITIONS["python"]["rules"]
...
def test_python_func_start_valid(payload, expected_name):
    assert_valid_match(PYTHON_RULES["func_start"], payload, expected_name, "python.func_start")


@pytest.mark.parametrize("payload", FUNCTION_CASES["invalid"])
def test_python_func_start_invalid(payload):
    assert_invalid_no_match(PYTHON_RULES["func_start"], payload, "python.func_start")


@pytest.mark.parametrize("payload,expected_name", FUNCTION_CASES["pathological"])
def test_python_func_start_pathological(payload, expected_name):
    assert_pathological_match(PYTHON_RULES["func_start"], payload, expected_name, "python.func_start")


def test_python_func_start_pep695_nested_bracket_regression():
    """
...
def test_python_func_start_redos_immunity():
...
def test_python_func_start_triple_quoted_string_lookalike_shielded_by_pipeline():
    """
...
def test_python_args_valid(payload, expected_name):
    assert_valid_match(PYTHON_RULES["args"], payload, expected_name, "python.args")


@pytest.mark.parametrize("payload", ARGS_CASES["invalid"])
def test_python_args_invalid(payload):
    assert_invalid_no_match(PYTHON_RULES["args"], payload, "python.args")


@pytest.mark.parametrize("payload,expected_name", ARGS_CASES["pathological"])
def test_python_args_pathological(payload, expected_name):
    assert_pathological_match(PYTHON_RULES["args"], payload, expected_name, "python.args")


def test_python_args_pep695_nested_bracket_regression():
    """Regression test for the same root-cause bug as func_start's own regression test above."""
    args = PYTHON_RULES["args"]
    m = args.search("def TargetFunc[T: Sequence[int]](x: T) -> T:")
    assert m, "PEP 695 nested-bracket generic args detection regressed"


def test_python_args_redos_immunity():
    """ReDoS sweep for the widened PEP 695 generic-parameter step-over."""
    args = PYTHON_RULES["args"]
...
def test_python_class_start_valid(payload, expected_name):
    assert_valid_match(PYTHON_RULES["class_start"], payload, expected_name, "python.class_start")


@pytest.mark.parametrize("payload", CLASS_CASES["invalid"])
def test_python_class_start_invalid(payload):
    assert_invalid_no_match(PYTHON_RULES["class_start"], payload, "python.class_start")


@pytest.mark.parametrize("payload,expected_name", CLASS_CASES["pathological"])
def test_python_class_start_pathological(payload, expected_name):
    assert_pathological_match(PYTHON_RULES["class_start"], payload, expected_name, "python.class_start")


def test_python_class_start_pep695_nested_bracket_base_capture_regression():
    """
    ...
    entirely there).
    """
    class_start = PYTHON_RULES["class_start"]
...
def test_python_class_start_redos_immunity():
    """ReDoS sweep for the widened PEP 695 generic-parameter step-over."""
    class_start = PYTHON_RULES["class_start"]
...
def test_python_dependency_capture_valid(payload, expected_path):
    assert_valid_dependency_match(
        PYTHON_RULES["_dependency_capture"], payload, expected_path, "python._dependency_capture"
    )


@pytest.mark.parametrize("payload", DEPENDENCY_CASES["invalid"])
def test_python_dependency_capture_invalid(payload):
    assert_invalid_no_match(PYTHON_RULES["_dependency_capture"], payload, "python._dependency_capture")


@pytest.mark.parametrize("payload,expected_path", DEPENDENCY_CASES["pathological"])
def test_python_dependency_capture_pathological(payload, expected_path):
    assert_pathological_dependency_match(
        PYTHON_RULES["_dependency_capture"], payload, expected_path, "python._dependency_capture"
    )


def test_python_dependency_capture_known_limitation_triple_quoted_string_lookalike_still_matches_at_regex_level():
    """
