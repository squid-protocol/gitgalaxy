...
_SOLIDITY_SIMPLE_CASES = [
    ...
    ("branch", "if (x) { return; }", "uint256 amount = 5;"),
    ("args", "function transfer(address to, uint256 amount) public {", "transfer(to, amount);"),
    ("structural_boundaries", "pragma solidity ^0.8.20;", "transfer(msg.sender, amount);"),
    ("func_start", "function transfer(address to) public {", "transfer(to, amount);"),
    ("class_start", "contract Token {", "// contract Token {"),
    ("safety", 'require(x > 0, "must be positive");', "requirement = 5;"),
    ("safety_bypasses", "unchecked { x++; }", "uncheckedValue = 5;"),
    ("high_risk_execution", "selfdestruct(payable(owner));", "selfDestructTimestamp = 0;"),
    ("api", "function foo() external {", "externally_verified = true;"),
    ("state_mutation", "balances[msg.sender] = amount;", "balances[msg.sender] == amount;"),
    ("dead_code", "// function foo() public {", "// just a note"),
    ("doc", "/// @notice does a thing", "// regular comment"),
    ("test", "assertEq(a, b);", "contestant = msg.sender;"),
    ("globals", "msg.sender", "messageHash = keccak256(data);"),
    ("generics", "mapping(address => uint256) public balances;", "uint256[] public balances;"),
    ...
    ("events", "emit Transfer(from, to, amount);", "eventuallySettled = true;"),
    ("pointers", "uint256[] memory arr;", "memoryPoolSize = 5;"),
    ("memory_alloc", "new uint256[](10);", "renewed = true;"),
    ("inline_asm", "assembly { let x := 1 }", "assemblyLine = 5;"),
    ("telemetry", 'console.log("debug");', "consoleWidget.render();"),
    ("explicit_casts", "uint256(x);", "uint256 x;"),
    ("panics_and_aborts", 'revert("error");', "revertedCount += 1;"),
    ("bitwise_ops", "a & b;", "a && b;"),
    ("sync_locks", "function foo() public nonReentrant {", "nonReentrantGuard = true;"),
    ("immutability_locks", "uint256 public constant MAX = 100;", "constantlyUpdated = true;"),
    ("cleanup", "delete balances[msg.sender];", "deletedAt = block.timestamp;"),
    ("encapsulation", "uint256 private secret;", "privatelyOwned = true;"),
    ("serialization_parsing", "abi.encode(a, b);", "abi.encodeWithSignature('foo()');"),
    ("regex_execution", "keccak256(abi.encodePacked(a, b));", "keccak256(data);"),
    ("time_date_logic", "uint256 x = block.timestamp + 1 days;", "blockNumber = block.number;"),
    ("ipc_rpc_bridges", "target.delegatecall(data);", "target.call(data);"),
