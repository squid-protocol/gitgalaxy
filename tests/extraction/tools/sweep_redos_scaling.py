        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call) and getattr(node.func, "id", "") == "assert_redos_immune":
                try:
                    ...
                    rule_key = (
                        rule_node.slice.value
                        if isinstance(rule_node, ast.Subscript)
                        else getattr(rule_node, "id", None)
                    )
                    if not rule_key or rule_key not in LANGUAGE_DEFINITIONS[lang]["rules"]:
                        continue
                    ...
                    ratios = [
                        durations[i] / durations[i - 1] for i in range(1, len(durations)) if durations[i - 1] > 0.0001
                    ]
                    total_tested += 1
                    ...
                    if ratios:
                        ...
                        if max_ratio >= 3.5 and durations[-1] > 0.05:
                            print(
                                f"[VULNERABLE] {lang}.{rule_key}: {max_ratio:.2f}x max ratio | {durations[-1]:.4f}s at N={sizes[-1]} | Ratios: {[f'{r:.1f}x' for r in ratios]}"
                            )
                            vuln_count += 1
                except Exception:
