...
_EXPLICIT_LABELS: dict[tuple[str, str], Optional[str]] = {
    ...
    # --- the mainframe family, new at corpus v1.3.0 (language-crucible#28).
    # These two groups are the first corpus files to take the OTHER side of
    # collisions that previously had only one claimant represented, which is
    # why they matter more than their file count suggests:
    (".asm", "hlasm/zopeneditor_sample"): "hlasm",  # every other .asm here is x86/6502/ARM
    (".sql", "db2_sql/db2unit"): "db2_sql",  # every other .sql here is sqlite-side
}
