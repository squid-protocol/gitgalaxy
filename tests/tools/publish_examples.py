...
def build_example(slug: str, corpus_name: str, what: str, out: Path, work: Path, head: str) -> dict[str, Any]:
    ...
    project = jtm.generate(clean, "default", {}, ex_work)
...
""", encoding="utf-8")  # fmt: skip
    return {"slug": slug, "corpus": entry, "what": what, "metrics": {"source": src_m, "java": java_m},
            **example_summary(dest / "java")}  # fmt: skip
...
def build_equivalence(out: Path, work: Path, finished: Optional[Path]) -> list[dict[str, Any]]:
    ...
    for case_file in sorted(eq.CASES.glob("*/case.json")):
        ...
        for extra in case_file.parent.glob("*.txt"):
...
`tests/tools/publish_examples.py`. The sources keep their licences: Apache-2.0 (CardDemo, zECS, Z Open
Editor sample), EPL-2.0 (CBSA, GenApp) and MIT (DSF). Each example's `PROVENANCE.md` says which applies.
"""
